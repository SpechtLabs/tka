...
func TestLoginHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:           "no cap rule",
			rule:           capability.Rule{},
			setup:          func(m *mock.MockTkaClient) k8s.TkaClient { return m },
		...
		},
		{
			...
			rule: rule,
			setup: func(m *mock.MockTkaClient) k8s.TkaClient {
				m.SignInFn = func(string, string, time.Duration) humane.Error { return humane.New("boom", "check server logs for details") }
				return m
			},
			expectedStatus:  http.StatusInternalServerError,
...
func TestGetLoginHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setup: func(m *mock.MockTkaClient) k8s.TkaClient {
				m.StatusFn = func(string) (*k8s.SignInInfo, humane.Error) {
					return &k8s.SignInInfo{Username: "alice", Role: "dev", ValidUntil: time.Now().Add(10 * time.Minute).Format(time.RFC3339), Provisioned: true}, nil
				}
		...
		},
		{
			...
			setup: func(m *mock.MockTkaClient) k8s.TkaClient {
				m.StatusFn = func(string) (*k8s.SignInInfo, humane.Error) { return nil, humane.New("kaput", "check server logs for details") }
